...
const Dashboard = () => {
    ...
    const AChangeHandler = index =>{
        ...
        axios.post('https://vjitstudentshubserver.cyclic.app/addUserA',{clgId:student.collegeId,namee:student.fullname,password:student.password,seatno:index}).then(
    ...
    const CChangeHandler = index =>{
        alert(index+" seat is allocated successfully");
        // console.log(student.fullname)
        axios.post('https://vjitstudentshubserver.cyclic.app/addUserC',{clgId:student.collegeId,namee:student.fullname,password:student.password,seatno:index}).then(
            res => { console.log(res.data); setQ(1) }
        )
    ...
    const EChangeHandler = index =>{
        alert(index+" seat is allocated successfully");
        // console.log(student.fullname)
        axios.post('https://vjitstudentshubserver.cyclic.app/addUserE',{clgId:student.collegeId,namee:student.fullname,password:student.password,seatno:index}).then(
            res => { console.log(res.data); setQ(1) }
        )
        ...
        <div>
            <center>
...

                {
                    z===2 ?
                    <div>
                        <h1 style={{border:"2px solid black",margin:"10px 20px",backgroundColor:"black",padding:"10px"}}>Stage / Screen</h1> <br />
                        ...
                        { SeatsdataA.map((arrayy,index) =>
                            ...
                            <div  style={{flex:1,display:"inline-flex"}}>
                                ...
                                <div  key={index}>
                                    ...
                                    { item !== "ASB" ?
                                    <div>
                                    <div style={{flex:1,border:"1px solid black",display:"inline-flex"}}>
            ...
            {usersA.length>=1 && <Navigate to="/profile2" /> }
            {usersC.length>=1 && <Navigate to="/profile2" /> }
            {usersE.length>=1 && <Navigate to="/profile2" /> }
