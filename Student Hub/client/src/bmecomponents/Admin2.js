...
const Admin2 = () => {
    ...
    const submitHandler = e =>{
        ...
        // }
        if(email && password){
            axios.post("https://vjitstudentshubserver.cyclic.app/verifyadminbme",data).then(
            res =>{ if(res.data === "success"){
                        setAuth(1);
                        setData({email:"",password:""})
                    }else{
                        alert(res.data)
    ...
    useEffect(()=>{
        ...
        axios.get("https://vjitstudentshubserver.cyclic.app/getquerybme").then(
        ...
        axios.get("https://vjitstudentshubserver.cyclic.app/getyears").then(
            ...
            axios.get("https://vjitstudentshubserver.cyclic.app/getauditorium").then(
                res => {
                    if(res.data.length>=1){
    ...
    const fileUpload = () =>{
        for(let i=0;i<selectedFiles.length;i++){
          ...
          axios.post('https://api.cloudinary.com/v1_1/drnndbow7/image/upload',formData).then(res =>{
            ...
            axios.post('https://vjitstudentshubserver.cyclic.app/addposter',{pic:res.data.url}).then(res =>{ console.log(res.data)  });
    ...
    const deleteproblem = index =>{
        console.log(index);
        axios.delete(`https://vjitstudentshubserver.cyclic.app/deleteproblembme/${index}`).then(
            res =>{ alert(res.data.message); setProblems(res.data.update) }
        )
    }

    const openregistration = () =>{
        axios.post('https://vjitstudentshubserver.cyclic.app/addopenregister',{dummy:2}).then(
            res => {alert(res.data.message); if(res.data.update.length>=1){
                                                setNexteventid(res.data.update[0]._id); 
                                                setX(1);
                                            } }
        )
    }
    ...
    const closeregistration = () =>{
        axios.delete(`https://vjitstudentshubserver.cyclic.app/deletecloseregister/${dummyid}`).then(
    ...
    const nexteventHandler = e =>{
        ...
        if(nexteventinfo){
            axios.post('https://vjitstudentshubserver.cyclic.app/addnextevent',{nextevent : nexteventinfo}).then(
    ...
    const updatenexteventHandler = e =>{
        ...
        if(updatenextevent){
            axios.put(`https://vjitstudentshubserver.cyclic.app/updatenextevent/${nexteventid}/${updatenextevent}`).then(
                res => {alert(res.data.message); if(res.data.update.length>=1){
                                                    setNexteventid(res.data.update[0]._id); 
                                                    setY(1);
                                                }}
    ...
    const yearssubmitHandler = e =>{
        ...
        if( (year1==="" || year1.length === 2) && (year2==="" || year2.length === 2 )&& (year3==="" || year3.length === 2) && (year4==="" || year4.length === 2) )
        {
            ...
            axios.post('https://vjitstudentshubserver.cyclic.app/addyears',years).then(
                res => {alert(res.data.message); if(res.data.update.length>=1){
                                                    setBatchs(res.data.update) 
                                                    setK(1);
                                                }}
            )
            console.log(years)
    ...
    const auditoriumsubmitHandler = e =>{
        ...
        if( (a1==="" || a1 === "A") && (a2==="" || a2 === "C" )&& (a3==="" || a3 === "E") )
        {
            ...
            axios.post('https://vjitstudentshubserver.cyclic.app/addauditorium',auditoriums).then(
                res => { alert(res.data.message); if(res.data.update.length>=1){
                                                    setAuditoriumlist(res.data.update) 
                                                    setL(1);
                                                } }
            )
            console.log(auditoriums)
    ...
    const deleteBatchsHandler = id =>{
        axios.delete(`https://vjitstudentshubserver.cyclic.app/deleteyears/${id}`).then(
            res => { alert(res.data.message); setK(0)
            } 
        )
    }

    const deleteAuditoriumHandler = id =>{
        axios.delete(`https://vjitstudentshubserver.cyclic.app/deleteauditorium/${id}`).then(
            res => { alert(res.data.message); setL(0) }
        )
    }
            ...
            {auth === 0 ?
                ...
                <div>
                    ...
                    <form className="fileupload">
                        ...
                        <input type="button" className="btn btn-primary submit" onClick={fileUpload} value='upload' />
                    </form>
                    </div>
                    ...
                    {y === 0 ? 
                    <div className="card bg-light" style={{width:"60%",boxShadow: "10px 10px 5px lightblue"}}>
                        <br />
                ...
                <div className="card bg-light" style={{width:"60%",boxShadow: "10px 10px 5px lightblue"}}>
                    ...
                    {  problems.length >= 1 ? 
                        <div>
                            { problems.map((item,value) => 
                                <div className="profile bg-light card " style={{"margin":"10px",width:"60%",boxShadow: "2px 2px 5px orange"}} key={value}>
